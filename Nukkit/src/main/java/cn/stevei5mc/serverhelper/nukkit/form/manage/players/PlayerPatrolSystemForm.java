...
public class PlayerPatrolSystemForm {
    ...
    public static void sendSelectPatrolPlayerUi(@NotNull Player admin) {
        ...
        custom.addElement(new ElementToggle(" 随机搜索 / 指定搜索"));
        ...
        custom.onResponded((form, player1) -> {
            if (Server.getInstance().getOnlinePlayers().size() >= 2) {
                Player targetPlayer = PlayerUtils.getRandomPlayer(form.getStepSliderResponse(4).getElementID(), player1, form.getDropdownResponse(5).getElementContent());

                if (form.getToggleResponse(0)) {
                    String target = !form.getInputResponse(3).equals("") ? form.getInputResponse(3) : form.getDropdownResponse(2).getElementContent();
                    if (target != null) {
